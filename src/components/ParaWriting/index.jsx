    ...
    <div className="paraWritingContainer">
      <motion.div className="paraWritingContainer--overlay" ref={ref} initial="hidden" animate={controls} variants={textVariants} onAnimationComplete={() => handleComplete()}>
        {text.split("").map((char, index) => (
          <motion.span key={index} variants={charVariants} onAnimationComplete={() => handleCharacterComplete(index)} ref={(el) => { if (el) spanRefs.current[index] = el }}>
            {char}
          </motion.span>
        ))}
        {sec &&
          sec.split("").map((char, index) => (
            <motion.span key={index} variants={charVariants} onAnimationComplete={() => handleCharacterComplete(index)} ref={(el) => { if (el) spanRefs.current[text.length + index] = el }} className="sec">
              {char}
            </motion.span>
          ))}
